...
#define HAL_CLOCK_STABLE()    //删除 st( while (CLKCONSTA != (CLKCONCMD_32MHZ | OSC_32KHZ)); )
...
__declspec(dllimport) void HAL_LED_BLINK_DELAY(void);

/*此处不做底层移植，下面宏定义不可以被使用，不过P0 P1口已经移植导出可以试试看
...
#define PREFETCH_ENABLE()     //删除st( FCTL = 0x08; )
#define PREFETCH_DISABLE()    //删除st( FCTL = 0x04; )
...
#define XNV_SPI_TX(x)               //删除st(U1CSR &= ~0x02; U1DBUF = (x);)
#define XNV_SPI_RX()                //删除U1DBUF
#define XNV_SPI_WAIT_RXRDY()        //删除st(while (!(U1CSR & 0x02));)
#define XNV_SPI_END()               //删除st(P1_3 = 1;)

// The TI reference design uses UART1 Alt. 2 in SPI mode.
#define XNV_SPI_INIT() //删除\
st( \
...
#ifndef HAL_AES_DMA
#define HAL_AES_DMA FALSE//没有DMA
#endif
