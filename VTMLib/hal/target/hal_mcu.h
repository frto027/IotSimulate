...
/* TODO: 对ioCC2530.h的移植：声明EA等变量，并作为DLL导出变量*/
#include "vtm.h"


/* ---------------------- mingw Compiler ---------------------- */
#define HAL_COMPILER_MINGW
//小端模式
#define HAL_MCU_LITTLE_ENDIAN()   1
//定义中断函数，并不是直接用f作为中断函数的
//第v个中断函数将以INT_v的形式给出
#define HAL_ISR_FUNC_DECLARATION(f,v)   asm (".section .drectve\n\t.ascii \" -export:INT_"#v"="#f"\"");void f(void)
...
/* 不用看门狗来Reset，调用一个函数 */
__declspec(dllimport) void HAL_SYSTEM_RESET(void);
